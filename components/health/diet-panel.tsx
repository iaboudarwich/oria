...
/**
 * Diet, as a tab inside the unified Health surface (Round 17). Meal logging by
 * photo (the dropzone) and by text/voice (TextLogForm), today + yesterday +
 * a 7-day view, and Ask Diet. Identical content to the former /dashboard/diet
 * page, minus the Topbar (the Health page owns it). Today's calorie balance
 * (intake here vs WHOOP burn) sits at the top; burnKcal is passed in.
 */
...
export async function DietPanel({ burnKcal }: { burnKcal: number | null }) {
  ...
  const yesterdayMeals = meals.filter((m) =>
    m.occurred_at ? sameDayInTz(new Date(m.occurred_at), yesterdayProbe, tz) : false,
  );
  ...
  const dailyAverageCalories =
    daysWithMeals > 0 ? Math.round(weekTotals.calories / daysWithMeals) : 0;
  const todayIntake = todayMeals.length > 0 ? Math.round(todayTotals.calories) : null;

  return (
    <div className="space-y-7">
      <CalorieBalanceCard intakeKcal={todayIntake} burnKcal={burnKcal} />
          ...
          <div className="rounded-2xl border border-line bg-surface-raised p-4">
            <div className="flex flex-wrap items-baseline gap-x-5 gap-y-1">
              ...
              <span className="text-[12.5px] text-ink-muted">
                ~{dailyAverageCalories.toLocaleString()} cal/day across {daysWithMeals} day
                {daysWithMeals === 1 ? "" : "s"}
              </span>
            </div>
              ...
              {week.map((d, i) => {
                ...
                return (
                  <li key={d.label + i} className="flex flex-1 flex-col items-center gap-1.5">
                    ...
                    />
                    <span className={`text-[10.5px] ${isToday ? "text-ink" : "text-ink-faint"}`}>
                      {d.label}
                    </span>
                  </li>
                );
      ...
      <p className="px-1 text-[11px] text-ink-faint">
        Nutrition estimates are approximate. Treat them as a calm reference, not medical advice.
      </p>
    </div>
  );
}

function DaySummary({ total, mealCount }: { total: DietTotals; mealCount: number }) {
  return (
    <div className="flex flex-col gap-2 rounded-2xl border border-line bg-surface-raised p-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      {total.protein_g > 0 || total.carbs_g > 0 || total.fat_g > 0 ? (
        <div className="flex items-baseline gap-4 text-[11.5px] text-ink-muted">
          {total.protein_g > 0 ? <span>{Math.round(total.protein_g)}g protein</span> : null}
          {total.carbs_g > 0 ? <span>{Math.round(total.carbs_g)}g carbs</span> : null}
          {total.fat_g > 0 ? <span>{Math.round(total.fat_g)}g fat</span> : null}
        </div>
      ) : null}
    </div>
  );
...
function MealRow({ meal: m }: { meal: DietMeal }) {
  const time = m.occurred_at
    ? new Date(m.occurred_at).toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" })
    : "·";
  const note =
  ...
  return (
    <li className="flex items-start gap-3 px-4 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        {note ? <p className="truncate text-[11.5px] text-ink-faint">{note}</p> : null}
      </div>
      <div className="text-right">
        {m.calories !== null ? (
          <p className="text-[13px] text-ink">{Math.round(m.calories).toLocaleString()} cal</p>
        ) : null}
    ...
    return (
      <div className="flex items-center justify-between rounded-2xl border border-line bg-surface-raised p-4">
        <div className="flex items-baseline gap-3">
          ...
          <span className="text-[13.5px] text-ink-muted">No meals logged yet</span>
        </div>
        <span className="text-[11.5px] text-ink-faint">Drop a meal photo above to start.</span>
      </div>
    );
  }
  const macros: Array<{ label: string; value: number; tint: string }> = [];
  if (total.protein_g > 0)
    macros.push({ label: "Protein", value: Math.round(total.protein_g), tint: "text-[#7a4a7a]" });
  if (total.carbs_g > 0)
    macros.push({ label: "Carbs", value: Math.round(total.carbs_g), tint: "text-[#7a5a2a]" });
  if (total.fat_g > 0)
    macros.push({ label: "Fat", value: Math.round(total.fat_g), tint: "text-[#3a6a8a]" });
  return (
        ...
        <div className="flex items-baseline gap-4 sm:gap-5">
          {macros.map((m) => (
            <span key={m.label} className="inline-flex items-baseline gap-1">
              <span className={`text-[14px] font-medium ${m.tint}`}>{m.value}</span>
...
function buildWeek(meals: DietMeal[], now: Date, tz: string): WeekBucket[] {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const cals = meals
      .filter((m) => (m.occurred_at ? sameDayInTz(new Date(m.occurred_at), probe, tz) : false))
