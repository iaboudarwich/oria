  ...
  return (
    <section>
      <div className="mb-2 flex items-baseline gap-2 px-1">
        <h2 className="text-eyebrow">What Oria remembers</h2>
        <span className="text-[11.5px] text-ink-faint">Scoped to {scope.label}</span>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-line bg-surface-raised">
        {memories.length === 0 ? (
          <p className="px-4 py-3 text-[12.5px] text-ink-faint">
            Nothing saved yet. Add a fact below and Oria will use it when answering questions about{" "}
            {scope.label}.
          </p>
        ) : (
                ...
                {editingId === m.id ? (
                  ...
                  >
                    <input
                      ...
                      autoFocus
                      maxLength={500}
                      className="block h-8 flex-1 rounded-md bg-canvas/60 px-2.5 text-[13px] text-ink outline-none placeholder:text-ink-faint focus:bg-canvas"
                    />
                    <button
                      type="submit"
                      disabled={pending || editDraft.trim().length === 0}
                      className="transition-base inline-flex h-8 items-center rounded-md bg-ink px-2.5 text-[11.5px] text-surface hover:bg-ink-soft disabled:opacity-50"
                    >
                      Save
                    </button>
                    <button
                      type="button"
                      onClick={cancelEdit}
                      className="transition-base inline-flex h-8 items-center rounded-md px-1.5 text-[11.5px] text-ink-faint hover:text-ink"
                    >
                      Cancel
                    </button>
                  </form>
                  ...
                  <>
                    ...
                    <button
                      ...
                      onClick={() => startEdit(m)}
                      disabled={pending}
                      className="transition-base text-[11.5px] text-ink-faint hover:text-ink disabled:opacity-50"
                    >
                      Edit
                    </button>
                    <button
                      ...
                      className="transition-base inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-md text-ink-faint hover:bg-canvas hover:text-claret disabled:opacity-50"
                    >
                      <CloseIcon size={12} />
                    </button>
                  </>
        ...
        >
          ...
          <button
            ...
            className="transition-base inline-flex h-9 items-center rounded-md bg-ink px-3 text-[12.5px] text-surface hover:bg-ink-soft disabled:opacity-50"
          >
      ...
      <p className="mt-2 px-1 text-[11px] text-ink-faint">
        Memories stay inside this section. Oria will not use them when answering questions in other
        sections.
      </p>
    </section>
  );
