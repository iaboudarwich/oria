  ...
  return (
    <section>
      ...
      <div className="mb-3 rounded-xl border border-line bg-surface-raised p-2">
        ...
        >
          ...
          <button
            ...
            className="inline-flex h-9 items-center gap-1 rounded-md bg-ink px-3 text-[12.5px] text-surface transition-base hover:bg-ink-soft disabled:opacity-50"
          >
      ...
      <p className="mt-2 px-1 text-[11px] text-ink-faint">
        Each row can go to a different section. The original image stays as
        the source.
      </p>
    </section>
  );
...
function ItemRow({
  ...
  const currentRef: { kind: "builtin" | "custom" | "review"; key: string } =
    it.section
    ...
    >
      <div className="min-w-0 flex-1">
        ...
        <p className="truncate text-[11.5px] text-ink-faint">
          {[amount, date, it.location, sectionName]
            .filter(Boolean)
            .join(" · ")}
        </p>
      </div>
