...
export async function Topbar({ title, subtitle, back }: TopbarProps) {
  ...
  const spaceName = spaceCrumb(ctx?.organization.name ?? null, ctx?.organization.kind);
  return (
    <header className="sticky top-0 z-20 -mx-4 mb-6 border-b border-line bg-canvas/85 px-4 pt-[max(1.25rem,env(safe-area-inset-top))] pb-3 backdrop-blur sm:-mx-6 sm:px-6 sm:pt-[max(1.5rem,env(safe-area-inset-top))] sm:pb-4 lg:-mx-10 lg:px-10">
      <div className="flex items-center justify-between gap-4">
        <div className="min-w-0 pl-12 lg:pl-0">
          ...
          <h1 className="text-title mt-0.5 text-ink">{title}</h1>
