    ...
    <section>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
        {sections.map((s) => {
          ...
          const Icon = isBuiltin ? SECTION_META[s.ref.key as Section].Icon : CustomSectionIcon;
          const count = isBuiltin ? (counts[s.ref.key as Section] ?? 0) : undefined;
          return (
            <Link
              key={`${s.ref.kind}-${s.ref.key}`}
              href={s.href}
              className="transition-base flex items-center gap-2.5 rounded-lg px-3 py-2.5 hover:bg-surface-raised"
            >
              <Icon size={14} />
