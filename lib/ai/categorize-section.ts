...
export async function inferSection(uploadId: string): Promise<CategoryResult> {
  ...
  if (!entity) {
    return {
      ...
      confidence: 0,
      reasoning: "No extracted entities found.",
    };
  }
  ...
  const customList =
    customSections.length > 0
      ? customSections.map((s) => `${s.name}: ${s.description ?? "custom section"}`).join("\n")
      : "(none)";
...
If nothing fits, use null. Use a builtin slug (lowercase) when it matches well. Use the exact custom section name when it fits better.`;
  ...
  try {
    ...
    const reasoning =
      typeof parsed.reasoning === "string" ? parsed.reasoning : "AI classification.";
    ...
    const matched = customSections.find((s) => s.name.toLowerCase() === sectionName.toLowerCase());
    if (matched) {
      return {
        section: null,
