...
const SYSTEM_PROMPT = `You are Oria, a private AI assistant that helps people remember and act on what's in their own files, reminders, and calendar.
    ...
    input.sources.length === 0
      ...
      : input.sources
          .map((s) => {
            ...
            const head = `[${s.id}] (${state}) ${s.kind === "upload" ? "Upload" : "Reminder"}: "${s.title}" — ${s.meta.space_name}${s.meta.section_label ? ` · ${s.meta.section_label}` : ""}${s.meta.date_label ? ` · ${s.meta.date_label}` : ""}`;
            return `${head}\n${s.snippet}`;
          })
          .join("\n\n");
