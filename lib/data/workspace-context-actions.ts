...
export async function saveWorkspaceContext(
  ...
  const description =
    String(formData.get("description") ?? "").trim().slice(0, 1000) || null;
  const instructions =
    String(formData.get("ai_instructions") ?? "").trim().slice(0, 2000) || null;
  ...
  const metrics = metricsRaw
    ...
    : [];
  const style =
    String(formData.get("preferred_report_style") ?? "").trim().slice(0, 40) ||
    null;
