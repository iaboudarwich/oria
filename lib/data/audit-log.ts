 ...
 */
export async function logAnonAuthFailure(input: { email: string; reason: string }): Promise<void> {
  try {
    const { ip, userAgent } = await readRequestFingerprint();
    const admin = createAdminClient();
    await admin.from("audit_log").insert({
...
export async function listRecentAuditEvents(userId: string, limit = 100): Promise<AuditEvent[]> {
  const admin = createAdminClient();
...
export function actionLabel(action: string): string {
  const map: Record<string, string> = {
    ...
    "reminder.updated": "Edited a reminder",
